...
static void uploadToMline(void * data, size_t len, uint8_t id);
...
static void receiveMline()
{
	if(getReceivedDataSpiDevSec(ECTM_ReceiveBuffer, ECTM_MESSAGE_SIZE))
	{
...
static void transmitMline()
{
	exlnk_closeHeader(&SendBuffer);

	setTransmitDataSpiDevSec(SendBuffer.data, SendBuffer.pt_data);

	if(transmitSpiDevSec())
		NeedToSend = 0;
	ECTM_SendData_Counter++;
	exlnk_clearSetHeader(&SendBuffer);
	//new
